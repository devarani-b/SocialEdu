...
router.get('/', function (req, res, next) {
    if (global.socialedu_settings.runOnce === true) {
        users.listAll(function (err, users) {
            if (users && users.length > 0) {
                ...
                fs.rename(path.join(__dirname, '../views/runOnce.hbs'), path.join(__dirname, '../views/runOnce.done.hbs'), function (err) {
                    ...
                    global.socialedu_settings.runOnce = false;
                    res.redirect('/');
                });
            } else {
...
router.post('/', function (req, res, next) {
    if (global.socialedu_settings.databasePath !== '' && global.socialedu_settings.databasePath !== undefined) {
        try {
            fs.mkdirSync(global.socialedu_settings.databasePath);
        } catch (errr) {
    ...
    user.create(function (err, userId) {
        ...
        fs.rename(path.join(__dirname, '../views/runOnce.hbs'), path.join(__dirname, '../views/runOnce.done.hbs'), function (err) {
            ...
            fs.mkdir(global.socialedu_settings.uploadPath, function (err) {
                if (err && err.message && err.message.indexOf('EEXIST') === -1) { return next(err); }
                fs.mkdir(global.socialedu_settings.uploadPath + '/archive', function (err) {
                    if (err && err.message && err.message.indexOf('EEXIST') === -1) { return next(err); }
                    fs.mkdir(global.socialedu_settings.uploadPath + '/notencoded', function (err) {
                        if (err && err.message && err.message.indexOf('EEXIST') === -1) { return next(err); }
                        fs.mkdir(global.socialedu_settings.mediaPath, function (err) {
                            if (err && err.message && err.message.indexOf('EEXIST') === -1) { return next(err); }
                            res.redirect('/');
                        });
                    });
